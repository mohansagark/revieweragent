...
function createGitHubClient(token = resolveGitHubToken()) {
  return new Octokit2({ auth: token });
}

// src/core/idempotency.ts
function reviewCommitMarker(headSha) {
  return `<!-- revieweragent-commit:${headSha} -->`;
}
function bodyWithMarker(summary, headSha) {
  return `${summary}

${reviewCommitMarker(headSha)}`;
}
function findReviewByMarker(reviews, headSha) {
...
function createGitHubReviewPort(octokit, owner, repo) {
  return {
    ...
    async updateReview(reviewId, pr, headSha, summary) {
...
var JOB_NAME = "revieweragent";

// src/platform/github/checks.ts
function createGitHubCheckPort(octokit, owner, repo) {
  return {
    async upsertCheck(headSha, conclusion, title, summary) {
...
var UnrecognizedConfigVersionError = class extends Error {
  constructor(found) {
    super(`Unrecognized .revieweragent.yml version: ${String(found)}`);
    this.name = "UnrecognizedConfigVersionError";
  }
};
var InvalidConfigYamlError = class extends Error {
  constructor(cause) {
...
function parseConfig(raw) {
  let doc;
  try {
    doc = (0, import_yaml.parseDocument)(raw, { strict: true });
...
function readEventPayload() {
  ...
  if (!path2) throw new Error("GITHUB_EVENT_PATH is not set \u2014 not running inside GitHub Actions");
  return JSON.parse(readFileSync(path2, "utf8"));
}
async function resolveEventContext(octokit, owner, repo) {
...
function checkLimits(config, files) {
...
function decideLimitOutcome(config, overLimit) {
...
function decodeEntities(text) {
  return text.replace(/&#x([0-9a-fA-F]+);/g, (_, hex) => String.fromCodePoint(parseInt(hex, 16))).replace(/&#(\d+);/g, (_, dec) => String.fromCodePoint(parseInt(dec, 10))).replace(/&([a-zA-Z]+);/g, (match2, name) => NAMED_ENTITIES[name] ?? match2);
}
...
var INVISIBLE_CHARS = new RegExp(
  [
    ...
    // ASCII control chars (not tab/LF/CR)
  ].join("|"),
  "gu"
);
...
function stripImageAltText(text) {
...
var UNTRUSTED_DATA_OPEN = "<UNTRUSTED_PR_DATA>";
...
var SYSTEM_PROMPT = `You are an automated code reviewer for a pull request.
...
function buildInstructionsPreamble(maintainerInstructions) {
...
async function runReview() {
  ...
  const instructions = existsSync(INSTRUCTIONS_PATH) ? readFileSync2(INSTRUCTIONS_PATH, "utf8") : void 0;
