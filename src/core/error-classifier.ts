...
export function classifyError(err: ClassifiableError): ErrorClass {
  switch (err.kind) {
    ...
    case "http_400":
      // SPEC.md §8/§9: api-key credit 400 never self-heals -> fail closed.
      // Subscription plan-quota 400 is outsider-burnable and refills -> skip.
      // Other subscription 400s (bad schema, invalid request) fail closed.
